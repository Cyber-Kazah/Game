...
int main()
    ...
    //сталкер
    Stalkers stalker = {400, 300, txLoadImage ("Stalker.bmp"), txLoadImage ("Stalkerleft.bmp"), txLoadImage ("Stalkerright.bmp"), txLoadImage ("Stalkersit.bmp")};
    int y0Stalker = 1500;
    //бандит
    ...
    //пуля
    Bullet  bul = {0, 0, false, 10, 10};


    //препядствие
    Barrier bar = {200, 600, 200, 350, true};
    Barrier bar2 = {350, 590, 260, 320, true};
    ...
    while (!GetAsyncKeyState (VK_ESCAPE))
    {
          ...
          txBitBlt (txDC(), 0, 0, 1920, 946, background);
            if (bar.visible)  bar.draw();
            if (bar.visible) bar.draw();
            if (bar2.visible) bar2.draw();
            ;
